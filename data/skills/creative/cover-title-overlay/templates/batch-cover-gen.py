...
for i, (title, car, accent, color, prefix) in enumerate(TRACKS):
    ...
    if generate_bg(prompt, bg_path):
        ...
        time.sleep(4)  # Rate-limit between API calls
    else:
...
time.sleep(4)  # Rate-limit before album cover API call
...
for prefix in [t[4] for t in TRACKS] + ["album"]:
    ...
    dst = os.path.join(tg_dir, f"{prefix}.jpg")
...
print("\nAlso archive covers: cp /opt/data/music/artwork/<album>/*_cover.png /opt/data/music/artwork/covers/")
